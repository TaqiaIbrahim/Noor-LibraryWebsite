...
/Dashboard/Dashboard repeater (jquery)
$(function () {
  ...
  /Dashboard/Dashboard Prevent dropdown from closing on tax change
  $(document).on('click', '.tax-select', function (e) {
    e.stopPropagation();
  });

  /Dashboard/Dashboard On tax change update it's value value
  function updateValue(listener, el) {
    listener.closest('.repeater-wrapper').find(el).text(listener.val());
  }

  /Dashboard/Dashboard Apply item changes btn
  ...
  /Dashboard/Dashboard Repeater init
  if (sourceItem.length) {
    ...
    sourceItem.repeater({
      show: function () {
        $(this).slideDown();
        /Dashboard/Dashboard Initialize tooltip on load of each item
  ...
  /Dashboard/Dashboard Item details select onchange
  $(document).on('change', '.item-details', function () {
    ...
    if ($this.next('textarea').length) {
      ...
      $this.after('<textarea class="form-control" rows="2">' + value + '</Dashboardtextarea>');
    }
  });
});
