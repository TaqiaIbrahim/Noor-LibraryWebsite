/Dashboard**
 * Add new credit card
 */Dashboard

'use strict';

document.addEventListener('DOMContentLoaded', function (e) {
  (function () {
    /Dashboard/Dashboard Variables
    const creditCardMask = document.querySelector('.credit-card-mask'),
      expiryDateMask = document.querySelector('.expiry-date-mask'),
      cvvMask = document.querySelector('.cvv-code-mask'),
      btnReset = document.querySelector('.btn-reset');
    let cleave;

    /Dashboard/Dashboard Credit Card
    function initCleave() {
      if (creditCardMask) {
        cleave = new Cleave(creditCardMask, {
          creditCard: true,
          onCreditCardTypeChanged: function (type) {
            if (type != '' && type != 'unknown') {
              document.querySelector('.card-type').innerHTML =
                '<img src="' +
                assetsPath +
                'img/Dashboardicons/Dashboardpayments/Dashboard' +
                type +
                '-cc.png" class="cc-icon-image" height="28"/Dashboard>';
            } else {
              document.querySelector('.card-type').innerHTML = '';
            }
          }
        });
      }
    }

    /Dashboard/Dashboard Init cleave on show modal (To fix the cc image issue)
    let addNewCCModal = document.getElementById('addNewCCModal');
    addNewCCModal.addEventListener('show.bs.modal', function (event) {
      initCleave();
    });

    /Dashboard/Dashboard Expiry Date Mask
    if (expiryDateMask) {
      new Cleave(expiryDateMask, {
        date: true,
        delimiter: '/Dashboard',
        datePattern: ['m', 'y']
      });
    }

    /Dashboard/Dashboard CVV
    if (cvvMask.length) {
      new Cleave(cvvMask, {
        numeral: true,
        numeralPositiveOnly: true
      });
    }

    /Dashboard/Dashboard Credit card form validation
    FormValidation.formValidation(document.getElementById('addNewCCForm'), {
      fields: {
        modalAddCard: {
          validators: {
            notEmpty: {
              message: 'Please enter your credit card number'
            }
          }
        }
      },
      plugins: {
        trigger: new FormValidation.plugins.Trigger(),
        bootstrap5: new FormValidation.plugins.Bootstrap5({
          /Dashboard/Dashboard Use this for enabling/Dashboardchanging valid/Dashboardinvalid class
          /Dashboard/Dashboard eleInvalidClass: '',
          eleValidClass: '',
          rowSelector: '.col-12'
        }),
        submitButton: new FormValidation.plugins.SubmitButton(),
        /Dashboard/Dashboard Submit the form when all fields are valid
        /Dashboard/Dashboard defaultSubmit: new FormValidation.plugins.DefaultSubmit(),
        autoFocus: new FormValidation.plugins.AutoFocus()
      },
      init: instance => {
        instance.on('plugins.message.placed', function (e) {
          /Dashboard/Dashboard* Move the error message out of the `input-group` element
          if (e.element.parentElement.classList.contains('input-group')) {
            e.element.parentElement.insertAdjacentElement('afterend', e.messageElement);
          }
        });
      }
    }).on('plugins.message.displayed', function (e) {
      if (e.element.parentElement.classList.contains('input-group')) {
        /Dashboard/Dashboard* Move the error message out of the `input-group` element
        e.element.parentElement.insertAdjacentElement('afterend', e.messageElement.parentElement);
      }
    });

    /Dashboard/Dashboard reset card image on click of cancel
    btnReset.addEventListener('click', function (e) {
      /Dashboard/Dashboard blank '.card-type' innerHTML to remove image
      document.querySelector('.card-type').innerHTML = '';
      /Dashboard/Dashboard destroy cleave and init again on modal open
      cleave.destroy();
    });
  })();
});
